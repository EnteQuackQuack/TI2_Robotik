...
### Begründung
- Jedes Signal lässt sich als Summe von Sinusschwingungen unterschiedlicher Frequenzen darstellen (Fouriertransformation).
- Ohne ausreichend hohe Abtastrate können **Oberschwingungen** (höhere Frequenzen) nicht erfasst werden → **Alias-Effekt**: Falsche Rekonstruktion des Signals.

#### Fouriertransformation
Mathematisches Verfahren zur Zerlegung eines Signals in seine Frequenzkomponenten.

#### Alias-Effekt
Fehlerhafte Interpretation eines Signals aufgrund zu niedriger Abtastrate, die zu einer falschen Rekonstruktion führt.
...
- Bei unbekanntem Signal: Physik des Systems analysieren (z. B. maximale Tasterfrequenz = 10 Hz → $f_{\text{abtast}} > 20$ Hz).

### 2. Wahl der Abtastrate
Beispiel Taster: Maximale manuelle Betätigung = 10 Hz → **Abtastrate ≥ 20 Hz** (Intervall ≤ 50 ms).
...
- Die Abtastrate ist eine **Echtzeitanforderung**:
  - Zu niedrige Raten führen zu Datenverlust.
  - Zu hohe Raten können **Prell-Effekte** (z. B. bei Tastern) oder unnötige Rechenlast verursachen.
